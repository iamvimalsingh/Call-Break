...
export const CenterPlayArea: React.FC<CenterPlayAreaProps> = ({
  ...
  return (
    <div
      ...
      className={`relative ${
        isBidding
          ? 'w-28 h-28 xs:w-34 xs:h-34 sm:w-40 sm:h-40 md:w-44 md:h-44 lg:w-48 lg:h-48 max-w-[200px] max-h-[200px]'
          : 'w-38 h-38 xs:w-44 xs:h-44 sm:w-52 sm:h-52 md:w-56 md:h-56 lg:w-60 lg:h-60 max-w-[250px] max-h-[250px]'
      } rounded-full bg-gradient-to-b from-[#08291a]/90 via-[#041d13]/85 to-[#02120b]/95 border-2 border-emerald-500/30 flex items-center justify-center shadow-[inset_0_0_40px_rgba(0,0,0,0.7),0_15px_35px_rgba(0,0,0,0.5)] ring-1 ring-emerald-400/20 select-none transition-all duration-300 shrink-0 mx-auto`}
    >
      {/* Decorative Outer Felt Rings and Compass Crosshairs */}
      <div className="absolute inset-1.5 sm:inset-3 rounded-full border border-emerald-400/15 pointer-events-none" />
      <div className="absolute inset-5 sm:inset-10 rounded-full border border-dashed border-emerald-500/10 pointer-events-none" />
      ...
      <div className="absolute top-1 xs:top-1.5 sm:top-2.5 z-10 flex flex-col items-center">
        <AnimatePresence mode="wait">
          {northPlayed ? (
            ...
            <div className="w-10 h-14 xs:w-11 xs:h-15 sm:w-13 sm:h-18 rounded-lg border border-dashed border-emerald-500/20 bg-emerald-950/20 flex items-center justify-center text-[8.5px] xs:text-[9.5px] sm:text-[10px] font-mono text-emerald-400/30">
        ...
        <AnimatePresence mode="wait">
          {isShowingCompleted && winnerPos ? (
            ...
            <div key="trick-active-disc" className="flex flex-col items-center">
              <div className="text-[8px] xs:text-[9px] sm:text-[11px] uppercase font-mono tracking-wider text-emerald-400 font-bold">
